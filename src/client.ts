...
function getBaseUri(server: Server = 'Server 1', config: Configuration): string {
  if (config.environment === Environment.Base) {
    if (server === 'Server 1') {
      return '{ scheme }://{ host }{ base_path }/api';
    }
  }
  if (config.environment === Environment.OpenAPIEnvNegosudProd) {
    if (server === 'Server 1') {
      return pathTemplate`${new SkipEncode(config.baseUrl)}/api`;
    }
  }
